...
**Webhook repost**:
The bot's replacement for a user message that contained a tracked link from a supported platform (currently `threads.com` / `threads.net`, where the whole query is stripped; `instagram.com` URLs carrying an `?igsh=…` share tracker, where only `igsh` is removed; `dcard.tw` URLs carrying a `?cid=…` campaign tracker, where only `cid` is removed; and `youtube.com` / `youtu.be` URLs carrying an `?si=…` share tracker, where only `si` is removed). The original is deleted; a webhook posts the cleaned version under the original poster's username and avatar. Tracked in the `webhook_reposts` table — the table interface lives in `core/webhook_reposts.py` — for 90 days (same window as the archive) so reactions can drive confirm/delete even on older posts; the row remembers the user's original message ID so a later ❌ surfaces an `/archive show`-able ID in the mod-log "Deleted" notice (the webhook repost itself isn't archived).
